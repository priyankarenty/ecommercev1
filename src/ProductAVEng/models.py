...
class ProductAVEngQuerySet(models.query.QuerySet):
	def featured(self):
		return self.all()
...
class ProductAVEngManager(models.Manager):
	def get_queryset(self):
		return ProductAVEngQuerySet(self.model, using=self._db)
...
class ProductAVEng(models.Model):
	...
	# 	super(ProductAVEng,self).save(*args, **kwarg)

	# def __str__(self):
	# 	return self.TotalPrice

	objects = ProductAVEngManager()
	pdobjects = DataFrameManager()

	def get_absolute_url(self):
		return "/products/aveng/{slug}/".format(slug=self.slug)
...
pre_save.connect(product_pre_save_receiver, sender=ProductAVEng)
